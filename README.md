...
Other configurations are Beat-specific:
...
Elasticsearch's attributes can be set as:
...
The Elasticsearch's instance from where Kibana must retrieve data can be set as:
...
You can also set Elasticdump version:
...
* `"id"`: default value is _secrets_, although it can be set to anything else on attribues.
...
* `"ca"`: array with certificate-authorities file content encoded in Base64. In mostly real use cases, it'll have only one element.
...
Kibana's configurations and object (searches, visualizations and dashboards) can be imported into a Kibana's running instance. The JSON files must be placed at `data_bags/kibana/`. All files in this directory are imported into Kibana.
